...
def simpleACOtraveltime(csv_file, m, ro, max_it, br_azur, alpha=1):
	
	folder, instances, best_cost= parser.read_best_solutions(csv_file)

	txt_ACO1 = './DZ3/ACO/ACO_traveltime.txt'
	txt_ACO2 = './DZ3/ACO/ACO_traveltime_solutions.txt'
	txt_ACO3 = './DZ3/ACO/ACO_optimal_sol_traveltime.txt'
	num_optimal_sol = 0
	valid_sol = 0
	...
	for i in range(len(instances)):
		...
		while it < max_it:
			...
			print("UPDATE TRAILS")
			updateTrails(ants[:br_azur], tsp, trails, matrix, full_name, False)
		...
		parser.write1(txt_ACO1, instanca, apsolutno, relativno, yn)
		if yn == "DA":
			num_optimal_sol = num_optimal_sol + 1
		print("writing 2nd file traveltime")
		parser.write2(txt_ACO2, instanca, best, best_eval)
		if best_eval != float('inf'):
			valid_sol = valid_sol + 1

		print("res: ", full_name, best, best_eval, scores)
	
	print(parser.write_num_optimal(txt_ACO3, num_optimal_sol, valid_sol))
	return num_optimal_sol


def simpleACOmakespan(csv_file, m, ro, max_it, br_azur, alpha=1):
	folder, instances, best_cost= parser.read_best_solutions(csv_file)
	txt_ACO1 = './DZ3/ACO/ACO_makespan.txt'
	txt_ACO2 = './DZ3/ACO/ACO_makespan_solutions.txt'
	txt_ACO3 = './DZ3/ACO/ACO_optimal_sol_makespan.txt'
...
def doWalk(tsp, trails, alpha=1):
	#cities = [x for x in range(len(tsp)+1)]
	#cities = cities[1:]
	cities = tsp
...
if __name__ == "__main__":
	...
	#--uncomment THIS
	print(simpleACOmakespan(csv_span, m, ro, max_it, br_azur, alpha=1))
	print(simpleACOtraveltime(csv_travel, m, ro, max_it, br_azur, alpha=1))
	#----------------
